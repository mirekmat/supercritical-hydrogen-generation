...
%    2017-08-03: Fix certain edge-cases; support for savefig(...,'compact') mode

% License to use and modify this code is granted freely to all interested, as long as the original author is
% referenced and attributed as such. The original author maintains the right to be solely associated with this work.

% Programmed and Copyright by Yair M. Altman: altmany(at)gmail.com
% $Revision: 1.02 $  $Date: 2017/08/03 15:39:17 $
...
function data = getFigData(figFilename)
    ...
    figData = load(figFilename,'-mat');

    % The file contains a struct with version info (will be 080000 in HG2)
    %data = data.hgS_070000;
    fn = fieldnames(figData);
    data = figData.(fn{1});
    if isempty(data)  % maybe savefig(...,'compact') file: hgS_080000 is empty, hgM_080000 is not
        try data = figData.(fn{2}); catch, end  % try to extract data from hgM_080000 (if available)
        try data = data.GraphicsObjects.Format3Data; catch, end  % hgM_080000 format
    end
end
...
function consolidatedData = processStruct(consolidatedData,data,parentTag)
    if isempty(data)
        consolidatedData = struct();
        return
    end
    try thisType  = data.type;       catch, thisType  = data.Type; end
    try dataProps = data.properties; catch, dataProps = get(data); end
    try thisTag = dataProps.Tag;     catch, thisTag = thisType;    end
    if isempty(thisTag),  thisTag = thisType; end %='tag'
    thisTag = strrep(thisTag,'.','');
    thisTag = [parentTag thisTag '_'];
    if ~isempty(parentTag)
        thisTag = strrep(thisTag, 'figure1_', '');
        thisTag = strrep(thisTag, 'figure_', '');  % hgM_080000 format
    end
    ...
    try consolidatedData = rmfield(consolidatedData, 'figure_Number'); catch, end

    children = []; try children = data.children; catch, children = data.Children; end %#ok<NASGU>
    for childIdx = 1 : length(children)
        consolidatedData = processStruct(consolidatedData, children(childIdx), thisTag);
    end
end
